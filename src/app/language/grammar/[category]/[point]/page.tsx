...
export default function GrammarPointPage({ params }: Params) {
  ...
  const related = point.related
    ...
    .map((p) => ({
      id: p.id,
      title: p.title,
      href: `/language/grammar/${p.category}/${p.slug}`,
    }));
  ...
  return (
    <div className="space-y-8">
      ...
      <nav aria-label="Breadcrumb" className="text-sm text-ink-muted dark:text-paper-300">
        <Link href="/language/grammar" className="hover:text-hanko dark:hover:text-hanko-light">
          Grammar
        </Link>
        <span className="mx-1.5">/</span>
        <Link
          href={`/language/grammar/${meta.slug}`}
