...
import { buildKanaDetail } from "@/lib/characters";
import { getKanaByScript } from "@/lib/data";
import { glass } from "@/lib/ui";

/**
 * The kana index. Shared by /language/kana and /language/kana/[char]; the second
 * passes `initialChar` so a direct link opens with that character's panel showing
 * over the table.
 */
export function KanaIndex({ initialChar }: { initialChar?: string }) {
  const hiragana = getKanaByScript("hiragana");
  const katakana = getKanaByScript("katakana");

  // Arrow-key order follows the tables: all hiragana, then all katakana.
  const details = [...hiragana, ...katakana].flatMap((group) => group.items).map(buildKanaDetail);
    ...
    <div className="space-y-10">
      ...
      <section className="space-y-3">
        <p className="text-sm text-ink-muted dark:text-paper-300">
          Select any character for its stroke order, position in the grid, related forms and example
          words. Inside the panel, ← and → move between characters.
        </p>
        <CharacterBrowser
          kind="kana"
          basePath="/language/kana"
          details={details}
